...
architecture behave of convolve_conv_s_axi is
    ...
    constant ADDR_OUT_R_BASE : INTEGER := 16#4000#;
    constant ADDR_OUT_R_HIGH : INTEGER := 16#5fff#;
    constant ADDR_KRNL_BASE  : INTEGER := 16#6000#;
    constant ADDR_KRNL_HIGH  : INTEGER := 16#600f#;
    ...
    signal int_krnl_address1   : UNSIGNED(1 downto 0);
    ...
    signal int_krnl_shift      : UNSIGNED(1 downto 0);
...
-- ----------------------- Instantiation------------------
-- int_in_r
int_in_r : convolve_conv_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 1250,
     AWIDTH   => log2(1250))
port map (
     ...
     q1       => int_in_r_q1);
-- int_out_r
int_out_r : convolve_conv_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 1250,
     AWIDTH   => log2(1250))
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_in_r_read = '0') and (int_out_r_read = '0') and (int_krnl_read = '0') else '0';
    RVALID    <= RVALID_t;
...
-- ----------------------- Memory logic ------------------
    ...
    -- krnl
    int_krnl_address0    <= SHIFT_RIGHT(UNSIGNED(krnl_address0), 2)(1 downto 0);
    int_krnl_ce0         <= krnl_ce0;
    int_krnl_we0         <= '0';
    int_krnl_be0         <= (others => '0');
    int_krnl_d0          <= (others => '0');
    krnl_q0              <= STD_LOGIC_VECTOR(SHIFT_RIGHT(int_krnl_q0, TO_INTEGER(int_krnl_shift) * 8)(7 downto 0));
    int_krnl_address1    <= raddr(3 downto 2) when ar_hs = '1' else waddr(3 downto 2);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (krnl_ce0 = '1') then
                    int_krnl_shift <= UNSIGNED(krnl_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
